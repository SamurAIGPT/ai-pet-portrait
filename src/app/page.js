...
import {
  ...
  FaPlus, FaCheck, FaExclamationTriangle, FaTrash
} from "react-icons/fa";
...
export default function StudioPage() {
  ...
  const [customPrompt, setCustomPrompt] = useState(
    "A beautiful, highly detailed digital painting of my pet as a majestic royal portrait."
  );
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden p-4 sm:p-6 lg:p-8 gap-6 min-h-0">
        ...
        <div className="w-full md:w-[45%] flex flex-col gap-6 md:overflow-y-auto pr-0 md:pr-1 min-h-0 flex-shrink-0">
          ...
          <div className="mt-1">
            ...
            <p className="text-xs text-zinc-500 mt-1 font-medium leading-relaxed">
              Use advanced AI technology to transform your pet photos into stunning art portraits. Upload up to 5 photos for the best results.
            </p>
          </div>
          ...
          <div className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-md flex flex-col gap-4 relative">
            <div className="flex justify-between items-center">
              <div>
                <h3 className="text-sm font-bold text-zinc-800">Your Pet Photos</h3>
                <p className="text-[10px] text-zinc-450 font-bold mt-0.5">Upload clear, high-resolution pet photos (1-5 images)</p>
              </div>
            ...
            {petImages.length > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {petImages.map((url, index) => (
                  <div key={index} className="relative aspect-square rounded-xl overflow-hidden border border-zinc-200 bg-zinc-50 group shadow animate-in fade-in duration-205">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={url} alt={`Uploaded Pet ${index + 1}`} className="w-full h-full object-cover" />
                    <button
                      onClick={() => removePetImage(index)}
                      className="absolute top-2 right-2 p-1.5 bg-black/60 hover:bg-red-650 text-white rounded-lg opacity-0 group-hover:opacity-100 transition-all shadow-md cursor-pointer"
                      title="Remove image"
                    >
            ...
            {petImages.length < 5 && (
              <div className="border-2 border-dashed border-zinc-200 hover:border-emerald-400 rounded-xl aspect-[16/10] flex flex-col items-center justify-center bg-zinc-50/50 p-6 text-center cursor-pointer transition-colors relative group">
                <input
                  type="file"
                ...
                {isUploading ? (
                  ...
                  <>
                    <div className="h-12 w-12 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-500 flex items-center justify-center mb-4 shadow-sm group-hover:scale-105 transition-transform duration-200">
                      <FaUpload className="text-lg" />
                    </div>
                    <span className="text-xs font-bold text-zinc-800">Upload Pet Photos</span>
                    <span className="text-[10px] text-zinc-450 font-bold max-w-[200px] mt-1.5 leading-relaxed">
                      Select one or multiple photos, or drag & drop (max 5 images)
                    </span>
                  </>
                )}
              </div>
          ...
          <div className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-md flex flex-col gap-4">
            <div>
              ...
              <textarea
                ...
                className="w-full text-xs font-medium text-zinc-700 bg-zinc-50 border border-zinc-200 focus:border-emerald-400 focus:bg-white rounded-xl p-3.5 outline-none resize-none transition-all shadow-inner h-24 leading-relaxed"
              />
            </div>
        ...
        <div className="w-full md:w-[55%] flex flex-col bg-white border border-zinc-200 rounded-3xl p-5 shadow-lg relative min-h-[350px] md:h-full overflow-hidden flex-shrink-0">
          ...
          <div className="flex justify-between items-center border-b border-zinc-100 pb-3.5 mb-4 flex-shrink-0">
            <div>
              <h3 className="text-sm font-bold text-zinc-800">Your Pet Portrait</h3>
          ...
          <div className="flex-1 rounded-2xl overflow-hidden border border-zinc-150 bg-zinc-50/50 flex flex-col items-center justify-center p-6 relative min-h-[220px]">
            ...
            {resultImage ? (
              ...
              <div className="text-center max-w-sm px-4">
                <div className="h-16 w-16 bg-emerald-50 border border-emerald-100 rounded-3xl flex items-center justify-center mx-auto mb-5 shadow-md">
                  <FaSpinner className="animate-spin text-2xl text-emerald-500" />
                </div>
                <h4 className="text-sm font-bold text-zinc-800">Transforming Pet Photos...</h4>
                <p className="text-[11px] text-zinc-500 mt-2 leading-relaxed font-medium">
                  We are blending your uploaded pet photos with your custom prompt. This usually takes around 8-15 seconds.
                </p>
              ...
              <div className="text-center max-w-sm px-4 py-8">
                ...
                <h4 className="text-sm font-bold text-zinc-800">Ready to Start</h4>
                <p className="text-[11px] text-zinc-450 mt-2 leading-relaxed font-medium">
                  Upload up to 5 pet photos, customize your art prompt, and hit generate to create a masterpiece.
                </p>
              </div>
            )}
          </div>
          ...
          {resultImage && (
            <div className="flex gap-3 mt-4 border-t border-zinc-100 pt-4 flex-shrink-0">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 py-3 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-700 hover:to-teal-600 text-white rounded-xl text-xs font-black shadow-lg shadow-emerald-500/10 cursor-pointer animate-in fade-in slide-in-from-bottom-2 duration-200"
              >
                <FaDownload />
